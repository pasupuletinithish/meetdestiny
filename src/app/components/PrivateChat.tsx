...
export const PrivateChat: React.FC = () => {
  ...
  const [travelerAvatar, setTravelerAvatar] = useState<string>('');
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // ← Fetch traveler's Google avatar
      const { data: profile } = await supabase
        .from('user_profiles')
        .select('avatar_url')
        .eq('user_id', traveler.user_id)
        .maybeSingle();
      if (profile?.avatar_url) setTravelerAvatar(profile.avatar_url);
          ...
          <div style={{ position: 'relative', flexShrink: 0 }}>
            {travelerAvatar ? (
              <img
                src={travelerAvatar}
                alt={traveler.name}
                style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(30,136,229,0.2)', boxShadow: '0 4px 12px rgba(30,136,229,0.2)' }}
                onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }}
              />
            ) : (
              <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg, #1E88E5, #1565C0)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(30,136,229,0.25)' }}>
                <span style={{ fontSize: 15, fontWeight: 700, color: '#fff' }}>{travelerInitials}</span>
              </div>
            )}
            {/* Online dot */}
            <div style={{ position: 'absolute', bottom: 1, right: 1, width: 11, height: 11, borderRadius: '50%', background: '#22c55e', border: '2px solid white', boxShadow: '0 1px 4px rgba(34,197,94,0.4)' }} />
          </div>

          {/* Name + profession */}
          <div style={{ flex: 1, minWidth: 0 }}>
            <h1 style={{ fontSize: 16, fontWeight: 800, margin: 0, color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{traveler.name}</h1>
            <p style={{ fontSize: 12, color: '#64748b', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{traveler.profession}</p>
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px 8px', scrollbarWidth: 'none', display: 'flex', flexDirection: 'column' }}>
        {messages.length === 0 ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
            {travelerAvatar && (
              <img src={travelerAvatar} alt={traveler.name}
                style={{ width: 64, height: 64, borderRadius: '50%', objectFit: 'cover', border: '3px solid rgba(30,136,229,0.2)', marginBottom: 12, boxShadow: '0 4px 16px rgba(30,136,229,0.15)' }} />
            )}
            ...
            <p style={{ fontSize: 12, color: '#94a3b8', margin: 0 }}>Your conversation is private and secure 🔒</p>
          </div>
        ) : (
          <div style={{ flex: 1 }}>
                  ...
                  {group.msgs.map((message, index) => {
                    ...
                    return (
                      <motion.div key={message.id}
                        ...
                        {/* Traveler avatar on their messages */}
                        {!isOwn && (
                          <div style={{ flexShrink: 0, width: 32, marginBottom: 2 }}>
                            {showAvatar ? (
                              travelerAvatar ? (
                                <img src={travelerAvatar} alt={traveler.name}
                                  style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover', border: '1.5px solid rgba(30,136,229,0.15)' }} />
                              ) : (
                                <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg, #1E88E5, #1565C0)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                  <span style={{ fontSize: 11, fontWeight: 700, color: '#fff' }}>{travelerInitials}</span>
                                </div>
                              )
                            ) : <div style={{ width: 32 }} />}
                          </div>
