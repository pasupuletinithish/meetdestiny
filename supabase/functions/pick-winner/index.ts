...
serve(async (req) => {
  ...
  try {
    ...
    const { data: contestType } = await supabase
      .from('contest_types').select('*').eq('day_of_week', dayOfWeek).maybeSingle()
    ...
    const { data: checkins } = await supabase
      .from('checkins').select('user_id, name, created_at').eq('vehicle_id', vehicle_id)
    ...
    if (manual_winner_id) {
      ...
      const { data: pings } = await supabase.from('pings').select('from_user_id, to_user_id')
      ...
      userIds.forEach(uid => {
        ...
        scores[uid] = sent.filter(id => received.includes(id)).length
      })
      const top = Object.entries(scores).sort((a, b) => b[1] - a[1])[0]
      if (top) { winnerId = top[0]; winnerScore = top[1] }
    } else if (contestType.criteria === 'messages_sent') {
      const { data: messages } = await supabase.from('lounge_messages').select('user_id')
        .eq('vehicle_id', vehicle_id).in('user_id', userIds)
      const scores: Record<string, number> = {}
      messages?.forEach(m => { scores[m.user_id] = (scores[m.user_id] || 0) + 1 })
      const top = Object.entries(scores).sort((a, b) => b[1] - a[1])[0]
      if (top) { winnerId = top[0]; winnerScore = top[1] }
    } else if (contestType.criteria === 'first_checkin') {
      const first = checkins.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())[0]
      winnerId = first.user_id; winnerScore = 1
    } else if (contestType.criteria === 'friends_added') {
      const { data: friends } = await supabase.from('friends').select('requester_id, receiver_id').eq('status', 'accepted')
    ...
    const { data: winnerProfile } = await supabase.from('user_profiles').select('name').eq('user_id', winnerId).maybeSingle()
    ...
    const { data: coupon } = await supabase.from('coupons').select('*').eq('is_used', false).limit(1).maybeSingle()

    if (!coupon) {
      await supabase.from('contest_winners').insert({
        user_id: winnerId, vehicle_id, contest_type: contestType.name, score: winnerScore,
        coupon_id: null, journey_date: new Date().toISOString().split('T')[0],
        winner_email: winnerEmail, winner_name: winnerName, email_sent: false,
      })
      return new Response(JSON.stringify({
        success: true, winner: { id: winnerId, name: winnerName, email: winnerEmail, score: winnerScore },
        contest: contestType.name, message: 'Winner picked but no coupons available!',
      }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }

    await supabase.from('coupons').update({ is_used: true, used_by: winnerId, used_at: new Date().toISOString() }).eq('id', coupon.id)

    await supabase.from('contest_winners').insert({
      user_id: winnerId, vehicle_id, contest_type: contestType.name, score: winnerScore,
      coupon_id: coupon.id, journey_date: new Date().toISOString().split('T')[0],
      winner_email: winnerEmail, winner_name: winnerName, email_sent: false,
    })

    await supabase.functions.invoke('send-winner-email', {
      body: { winner_name: winnerName, winner_email: winnerEmail, coupon_code: coupon.code,
        contest_type: contestType.name, contest_description: contestType.description, vehicle_id }
    })

    return new Response(JSON.stringify({
      success: true, winner: { id: winnerId, name: winnerName, email: winnerEmail, score: winnerScore },
      contest: contestType.name, coupon: coupon.code,
    }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
